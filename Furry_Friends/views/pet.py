...
def ai_profile():
    ...
    if 'login' not in asd:
        ...
        f.filename = newname
        f.save(f'./Furry_Friends/samples/inputs/{newname}')

        os.system("python ./Furry_Friends/profile_process.py --checkpoint ./Furry_Friends/weights/generator_celeba_distill.pt --device cpu")

        s3.upload_file(Filename=f"./Furry_Friends/samples/results/{newname}", Bucket=AWS_S3_BUCKET_NAME, Key=newname)
        img_url = f"https://{AWS_S3_BUCKET_NAME}.s3.{AWS_S3_BUCKET_REGION}.amazonaws.com/{newname}"

        os.remove(f"./Furry_Friends/samples/inputs/{newname}")
        os.remove(f"./Furry_Friends/samples/results/{newname}")
